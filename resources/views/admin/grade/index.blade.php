        ...
        <div class="row">
            <div class="col-sm-12">
                    <h5 class="page-header">
                        <a href="{{ route('addGrade') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg> Add a Grade
                        </a>
                    </h5>
                    ...
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        @foreach($gradelist as $row)
                                <tr>
                                    ...
                                    <td data-title="">{{ $row->point}}</td>
                                    <td data-title="">{{ $row->gradefrom}}</td>
                                    <td data-title="">{{ $row->gradeupto}}</td>
                                    <td data-title="">{{ $row->note}}</td>
                                    <td data-title="Action">
                                    <a href="{{route('editGrade',['categoryId'=> $row->id])}}" class="btn btn-warning btn-xs mrg" data-placement="top" data-toggle="tooltip" data-original-title="Edit">Edit</i>
            ...
            buttons: {
                deleteUser: {
                    ...
                    action: function() {
                        $.ajax({
                            type: "GET",
                            url: "{{url('admin/grade/delete')}}?categoryId=" + categoryId,
